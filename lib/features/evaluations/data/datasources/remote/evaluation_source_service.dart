...
class EvaluationSourceService implements IEvaluationSource {
  ...
  Future<List<Evaluation>> getByCategoryID(String categoryId) async {
    ...
    final response = await httpClient.get(
      Uri.parse(
        "$_apiBaseUrl/$_databaseName/read?tableName=Evaluations&categoryID=$categoryId",
      ),
    ...
    if (response.statusCode == 200) {
      ...
      logError(
        "Failed to fetch evaluations. Status code: ${response.statusCode}",
      );
      return [];
    }
  }
  ...
  Future<List<Evaluation>> getAllEval() async {
    ...
    if (response.statusCode == 200) {
      ...
      logError(
        "Failed to fetch all evaluations. Status code: ${response.statusCode}",
      );
      return [];
    }
  }
  ...
  Future<void> createEvaluation({
    ...
    logInfo(
      "Creating evaluation with name: $name, categoryId: $categoryId, visibility: $visibility, creationDate: $creationDate",
    );
    ...
    if (response.statusCode != 201) {
      logError(
        "Failed to create evaluation. Status code: ${response.statusCode}",
      );
  ...
  @override
  Future<List<String>> getScoresByCategoryID(
    String categoryId,
    String evaluationId,
  ) async {
    final response = await httpClient.get(
      Uri.parse(
        "$_apiBaseUrl/$_databaseName/read?tableName=EvaluationScore&categoryID=$categoryId&evaluationID=$evaluationId",
      ),
      headers: {'Authorization': 'Bearer $_authToken'},
    );
    ...
    if (response.statusCode == 200) {
      ...
      logError(
        "Failed to fetch scores by category. Status code: ${response.statusCode}",
      );
      return [];
    }
  }
  ...
  Future<List<String>> getScoresByEvaluationID(String evaluationId) async {
    final response = await httpClient.get(
      Uri.parse(
        "$_apiBaseUrl/$_databaseName/read?tableName=EvaluationScore&evaluationID=$evaluationId",
      ),
      headers: {'Authorization': 'Bearer $_authToken'},
    );
    logInfo(
      "Scores by evaluation fetch response status: ${response.statusCode}",
    );
    ...
    if (response.statusCode == 200) {
      ...
      logError(
        "Failed to fetch scores by evaluation. Status code: ${response.statusCode}",
      );
      return [];
    }
  }
  ...
  @override
  Future<List<String>> getScoresByGroupID(
    String groupId,
    String evaluationId,
  ) async {
    final response = await httpClient.get(
      Uri.parse(
        "$_apiBaseUrl/$_databaseName/read?tableName=EvaluationScore&groupID=$groupId&evaluationID=$evaluationId",
      ),
      headers: {'Authorization': 'Bearer $_authToken'},
    );
    ...
    if (response.statusCode == 200) {
      ...
      logError(
        "Failed to fetch scores by group. Status code: ${response.statusCode}",
      );
      return [];
    }
  }
  ...
  Future<List<String>> getUserScores(String userId, String evaluationId) async {
    final response = await httpClient.get(
      Uri.parse(
        "$_apiBaseUrl/$_databaseName/read?tableName=EvaluationScore&userID=$userId&evaluationID=$evaluationId",
      ),
      headers: {'Authorization': 'Bearer $_authToken'},
    );
    ...
    if (response.statusCode == 200) {
      ...
      logError(
        "Failed to fetch user scores. Status code: ${response.statusCode}",
      );
      return [];
    }
  }
  ...
  Future<List<String>> getAllUserScores(String userId) async {
    final response = await httpClient.get(
      Uri.parse(
        "$_apiBaseUrl/$_databaseName/read?tableName=EvaluationScore&userID=$userId",
      ),
      headers: {'Authorization': 'Bearer $_authToken'},
    );
    ...
    if (response.statusCode == 200) {
      ...
      logError(
        "Failed to fetch all user scores. Status code: ${response.statusCode}",
      );
      return [];
    }
  }
  ...
  Future<List<Evaluation>> getUserEvaluations(String userId) async {
    logInfo("Fetching evaluations for user ID: $userId");
    GroupsController groupController = Get.find();

    logInfo("Fetching groups for user ID: $userId");
    final userGroups = await groupController.getUserGroups(userId);
    logInfo("User groups fetched: $userGroups");

    final List<Evaluation> userEvaluations = [];
    logInfo("Processing user groups to fetch evaluations");

    for (var group in userGroups) {
      logInfo("User group: ${group.id}, categoryID: ${group.categoryID}");
      final evaluations = await getByCategoryID(group.categoryID);
      for (var eval in evaluations) {
        logInfo(
          "Evaluation for user $userId: ${eval.name} in category ${group.categoryID}",
        );
